...
Define a method              def <methodName>([args])
Define a constructor         def __init__([args])
Create an object from class  <obj> = <ClassName>([args])
    ...
    def display_info(self, courses_list):
        ...
        if self.__marks:
            ...
            for i, course in enumerate(courses_list, 1):
                print(f"   {i}. {course.get_id()} - {course.get_name()}")   # enumerate is used to iterate through the courses_list along with the index of each element.
        ...
        for _ in range(idx_course):
            ...
            while True:
                if any(ext_course.get_name() == course['name'] for ext_course in self.get_courses()):
                    ...
                    continue
                break
            new_course = Course(course['id'], course['name'])
